...
% MADE THIS CHANGE: RUI

try 
    warning('This is made by Rui in inv_solve_diff_GN_one_step')
    inv_model.inv_solve_core.do_pcg; %check if this is not an error

    if inv_model.inv_solve_core.do_pcg
    ...
    % Set max_iterations
    max_iterations = 1; %difference solve;
    % Set lambda
    lambda = inv_model.hyperparameter.value^2; %squared because it's stored as the sqrt after GCV

    if isfield(inv_model,'RtR_prior') && isa(inv_model.RtR_prior,'function_handle') ...
            && strcmp(func2str(inv_model.RtR_prior),'prior_tikhonov')
        RtR = @(x,j) x;
    else
        error('Should be tykhonov here! for now');
    end
    
    % Set x0
    x0 = zeros(length(img_bkgnd.elem_data),1);
