classdef test3DMagneticFieldComputation < matlab.unittest.TestCase
    ...
    methods(Test)
        function test3DCylinder(test_case)
            ...
            B_num = compute_biot_savart_numerical(fmdl,J,R,z1,z2,mu0);
        ...
        %%
        function test3DDifferentMeasurementAxis(test_case)
            %% Prepare workspace
            % Get the full path of the current script
            fullpath = mfilename('fullpath');
            % Extract just the folder
            ...
            % This does not need sensors.axes defined, only sensors.position
            mu0 = model_parameters.mu0;
            fmdl = compute_geometry_matrices(fmdl,mu0);

            %% Define the current and cylinder parameters
            J = [0 0 -1];
            % Cylinder parameters
            R  = model_parameters.radius;
            z1 = 0;
            ...
            %% Same positions, different measurement axis. Compute magnetic fields.
            sensor_axes_cylindrical = construct_sensor_axis(sensor_locations,'cartesian');
            for m = 1:num_sensors
                sensors(m).axes = sensor_axes_cylindrical(m);
            end
            fmdl.sensors = sensors;

            figure;
            plot_mdl_sensors_axes(fmdl)

            %Solve forward model with my solver

            % Assign homogeneous conductivity, re-assign fmdl because of
            % the updated sensors field
            img = mk_image(fmdl,1.0,'Homogeneous 3D cylinder');
            img.fwd_solve.get_all_meas = 1;

            %Solve forward model with my solver
            [data_cartesian,~] = fwd_solve_mdeit(img);

            %% Compute magnetic fields with numerical integration of Biot Savart Law
            B_num_cartesian = compute_biot_savart_numerical(fmdl,J,R,z1,z2,mu0);

            %% Same positions, different measurement axis. Compute magnetic fields.
            sensor_axes_cylindrical = construct_sensor_axis(sensor_locations,'cylindrical');
            for m = 1:num_sensors
                sensors(m).axes = sensor_axes_cylindrical(m);
            end
            fmdl.sensors = sensors;
            ...
            %Solve forward model with my solver
            [data_cylindrical,~] = fwd_solve_mdeit(img);
            
            %% Compute magnetic fields with numerical integration of Biot Savart Law
            B_num_cylindrical = compute_biot_savart_numerical(fmdl,J,R,z1,z2,mu0);
            
            %% B vectors must be the same up to numerical error

            % Test for numerical integration
            for m = 1:num_sensors
                % Cylindrical axis, since the last update of sensors was
                % setting them as cylindrical
                axes = sensors(m).axes;
                
                Bm = ...
                    B_num_cylindrical(m,1).*axes.axis1+...
                    B_num_cylindrical(m,2).*axes.axis2+...
                    B_num_cylindrical(m,3).*axes.axis3;
                
                pass = norm(Bm-B_num_cartesian(m,:),'inf')./norm(B_num_cartesian(m,:),'inf')<test_case.test_parameters.error_thresh;
                test_case.assertTrue(pass);
            end

            % Test for r-matrix computation
            for m = 1:num_sensors
                % Cylindrical axis, since the last update of sensors was
                % setting them as cylindrical
                axes = sensors(m).axes;
                
                B_rthetaz = [data_cylindrical.Bx(m),data_cylindrical.By(m),data_cylindrical.Bz(m)];

                Bm = ...
                    B_rthetaz(1).*axes.axis1+...
                    B_rthetaz(2).*axes.axis2+...
                    B_rthetaz(3).*axes.axis3;

                B_xyz =  [data_cartesian.Bx(m),data_cartesian.By(m),data_cartesian.Bz(m)];

                pass = norm(Bm-B_xyz,'inf')./norm(B_xyz,'inf')<test_case.test_parameters.error_thresh;
                test_case.assertTrue(pass);
            end
        end
    end
...
%% FUNCTIONS:
function plot_mdl_sensors_axes(fmdl)
%% Plot the sensors and sensor axis to see if everything is correct
show_fem(fmdl);
plot_sensors(fmdl)
...
x_min = 2*min(fmdl.nodes(:,1));
x_max = 2*max(fmdl.nodes(:,1));

y_min = 2*min(fmdl.nodes(:,2));
y_max = 2*max(fmdl.nodes(:,2));
...
function B_num = compute_biot_savart_numerical(fmdl,J,R,z1,z2,mu0)
% Compute magnetic fields with numerical integration of Biot Savart Law in
% cylindrical coordinates

n_sensors = numel(fmdl.sensors);
B_num = zeros(n_sensors,3);
