% Lets structure an mdeit model as the following structure:

% mdl:

% - mesh ( stores the mesh data)

% - eletrodes/stimulation (stores how the electrodes relate to the mesh and
% and the stimulation/measurement patterns)

% - physical_constants (might be useful to store characteristic units
% here?)

% - sensors (stores sensor positions, orienations and type - magnetometer or gradiometer)

% - geometry (stores the pre-computed geometry matrices here?)

% - file_name

% Notes:
% The mesh and geometry data are only computed once. Afterwards they are
% stored in model_folder, and they can be loaded to avoid recomputation.
% The mdl should load only the necessary mesh/geometry if model_parameters
% calls for it. 

% First, we should split model_parameters into mesh_parameters and
% geometry_parameters.

function [model_parameters,mdl] = ...
    mk_single_mdeit_model_(model_parameters,model_folder,opts)
...
function [model_parameters_array_new,fmdl_cell_array] = ...
    mk_mdeit_model_v2(model_parameters_array,model_folder,options)
