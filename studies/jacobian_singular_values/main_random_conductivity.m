...
num_of_electrodes_per_ring_vector = [4];
random_conductivity_seeds = [1 2 3 4];

[R, E, S] = ndgrid(num_of_rings_vector, num_of_electrodes_per_ring_vector,random_conductivity_seeds);
parameter_values = [R(:)'; E(:)';S(:)'];

num_of_sensors = parameter_values(1,:).*parameter_values(2,:);
parameter_values(4,:) = num_of_sensors;
...
model_parameters_array = ...
    ...
    {'numOfRings','numOfElectrodesPerRing','randomConductivitySeed','numOfSensors'},...
    parameter_values,...
    [], ...
...
for i = 1:length(fmdls)
    ...
    rng(random_seed_array(i),'twister');
    n_elems = size(fmdl.elems,1);
    elem_data = background_conductivity*ones(n_elems,1) + ...
        0.5*background_conductivity*(2*(rand(n_elems,1)-0.5));

    imgh = mk_image_mdeit(fmdl,elem_data);
