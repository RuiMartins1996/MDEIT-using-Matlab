...
% Use a different mesh resolution for optimization of sensor configuration
...
% Recon model
if ~exist(fullfile(script_folder,"\model\model_recon.mat"),'file')
    shape = {height,                        % height
        {thorax},                       % contours
    ...
    elec_shape = [electrode_radius,               % radius 
    ...
    subplot(1,3,1)
    show_fem_transparent_edges(fmdl_recon);
    drawnow;
end
...
% 
%     string_total = sprintf('solid ball = %s -maxh=%.2f;',string,maxsz_mesh);
% 
%     extra={'ball',string_total};
% 
%     fmdl = ng_mk_extruded_model(shape, elec_pos, elec_shape,extra);
% 
%     % EIDORS WEIRD ERROR: For some reason, NETGEN and EIDORS thinks that the introduced anomaly
%     % is an electrode. Play with the position and radius of the anomaly to
%     % avoid this
% 
% 
%     % Complete models for MDEIT
...
fmdl_inhom.stimulation = stimulation;
...
% imgi = add_material_properties(imgi, [1.0 2.0 0.3 0.3]); %background, anomaly, right lung, left lung
imgi = add_material_properties(imgi, [1.0 0.4 0.4]); %background, anomaly, right lung, left lung

img_recon = mk_image_mdeit(fmdl_recon,1.0);

imgi.calc_colours.ref_level = 1;
...
lambda_vector = logspace(-30,1,50);
[lambda_opt,dx] = l_curve_method_new(J,dy_opt,lambda_vector);

variance_prior = variance_noise/lambda_opt;

%% Identity prior (no prior knowledge of dsigma)

%% Build smooth prior encoding the lung information
...
if ~exist('data/sensor_positions_opt.mat','file')
    ...
    perm = [bg_elem_ids; lung_elem_ids];
    ...
    Gamma_prior = variance_prior*speye(n_elem);
    inv_Gamma_prior = Gamma_prior \ speye(size(Gamma_prior));
    inv_Gamma_prior_3_axis = inv_Gamma_prior;
end
...
q_to_x_xi = @(q) map_q_to_x_xi(q,rmin,rmax,theta0,z0);
...
if ~exist('data/sensor_positions_opt.mat','file')
    for k = 1:num_initial_conditions
        ...
        sensor_positions_0 = sensor_positions_0;
...
delta = 0.05;
sensor_positions_boundary = find_sensor_positions_boundary(fmdl_hom,[0,0,height/2],delta);

%%
figure

subplot(1,num_initial_conditions+2,1)
...
for n = 1:num_initial_conditions
    subplot(1,num_initial_conditions+2,n+1)
...
% A = @(x) M(img_recon,x); DEBUG
% imgtemph = img_recon; DEBUG
...
Gamma_post_no_opt = Gamma_prior-(Gamma_prior*J_3axis_no_opt.')/(J_3axis_no_opt*Gamma_prior*J_3axis_no_opt.'+Gamma_noise_3_axis)*J_3axis_no_opt*Gamma_prior;
...
posterior_mean_no_opt = Gamma_post_no_opt*(...
    J_3axis_no_opt.'*inv_Gamma_noise*dy_init);
%% 
...
Gamma_post_opt = Gamma_prior-(Gamma_prior*J_3axis_opt.')/(J_3axis_opt*Gamma_prior*J_3axis_opt.'+Gamma_noise_3_axis)*J_3axis_opt*Gamma_prior;

posterior_mean_opt = Gamma_post_opt*(...
    J_3axis_opt.'*inv_Gamma_noise*dy_opt);
...
Gamma_post_bdy = Gamma_prior-(Gamma_prior*J_3axis_bdy.')/(J_3axis_bdy*Gamma_prior*J_3axis_bdy.'+Gamma_noise_3_axis)*J_3axis_bdy*Gamma_prior;

posterior_mean_bdy = Gamma_post_opt*(...
    J_3axis_bdy.'*inv_Gamma_noise*dy_bdy);
