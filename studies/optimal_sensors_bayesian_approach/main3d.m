...
model_parameters_temp.maxsz = model_parameters.radius/5;
...
% Sanity check
q = x_to_q_xi_theta(x0);
x = q_to_x_xi_theta(q);
assert(norm(x-x0)<1e-5,'Unexpected');

% Sanity check
q0 = sensor_locations_to_vector_xi_theta(sensor_locations_0);
sensor_locations_new = vector_to_sensor_locations_xi_theta(q0);
assert(norm(sensor_locations_new-sensor_locations_0)<1e-5,'Unexpected');

% Sanity check
sensor_locations = vector_to_sensor_locations_xi_theta(q0);
q_new = sensor_locations_to_vector_xi_theta(sensor_locations);
assert(norm(q0-q_new)<1e-5,'Unexpected');


jac_coor_transf_xi_theta = @(s) compute_jacobian_coordinate_transformation_xi_theta(s,rmin,rmax);

jac_coor_transf_theta = @(s) compute_jacobian_coordinate_transformation_theta(s);

% DBEUG!
jac_coor_transf_test = @(s) compute_jacobian_coordinate_transformation_test(s);
...
% img_out = optimize_sensor_configuration(imgh_recon,inv_Gamma_prior_3_axis,inv_Gamma_noise_3_axis,...
%     jac_coor_transf_xi_theta,q_to_x_xi_theta,x_to_q_xi_theta,'a-opt','mdeit3',3,options);

img_out = optimize_sensor_configuration(imgh_recon,inv_Gamma_prior_3_axis,inv_Gamma_noise_3_axis,...
    jac_coor_transf_theta,q_to_x_theta,x_to_q_theta,'a-opt','mdeit3',3,options);

% img_out = optimize_sensor_configuration(imgh_recon,inv_Gamma_prior_3_axis,inv_Gamma_noise_3_axis,...
%     jac_coor_transf_r_theta,q_to_x_r_theta,x_to_q_r_theta,'a-opt','mdeit3',3);
...
sensor_locations_n = fetch_sensor_locations(img_out);

% Reconstruct with old sensor postions
fprintf('Solving fmdl for initial sensor positions\n')
imgtempi = imgi;
imgtempi = assign_sensor_locations(imgtempi,sensor_locations_0);
imgtemph = imgh;
imgtemph = assign_sensor_locations(imgtemph,sensor_locations_0);

yi_init = fwd_solve_mdeit_local(imgtempi);
yi_init = [yi_init.Bx(:);yi_init.By(:);yi_init.Bz(:)];
yh_init = fwd_solve_mdeit_local(imgtemph);
yh_init = [yh_init.Bx(:);yh_init.By(:);yh_init.Bz(:)];

% DEFINE THE NOISE AMPLITUDE WITH RESPECT TO THE INITIAL SENSOR
% CONFIGURATION SOLUTIOON
noise_amplitude = 1e-1*max(abs(yh_init-yi_init));

dy_old = add_measurement_noise_difference_absolute(yi_init,yh_init,noise_amplitude);


%Reconstruct with new sensor postions
fprintf('Solving fmdl for optimal sensor positions\n')
imgtempi = imgi;
imgtempi = assign_sensor_locations(imgtempi,sensor_locations_n);
imgtemph = imgh;
imgtemph = assign_sensor_locations(imgtemph,sensor_locations_n);

yi_opt = fwd_solve_mdeit_local(imgtempi);
yi_opt = [yi_opt.Bx(:);yi_opt.By(:);yi_opt.Bz(:)];
yh_opt = fwd_solve_mdeit_local(imgtemph);
yh_opt = [yh_opt.Bx(:);yh_opt.By(:);yh_opt.Bz(:)];
dy = add_measurement_noise_difference_absolute(yi_opt,yh_opt,noise_amplitude);

subplot(1,2,1)
hold on
plot(dy_old,'r.')
plot(yi_init-yh_init,'b')
grid on;grid minor;
axis square
box on;
legend('Noisy','Real')
title('Data initial sensor positions','interpreter','latex')

subplot(1,2,2)
hold on
plot(dy,'r.')
plot(yi_opt-yh_opt,'b')
grid on;grid minor;
axis square
box on;
legend('Noisy','Real')
title('Data opt sensor positions','interpreter','latex')
% (NOTE: Signal is weaker when sensors are far away)!!!!!!!!!!!
drawnow
...
[lambda_opt,sigma_gcv_opt] = generalized_cross_validation(J_3axis_opt,dy,lambda_vector);
fprintf('\t For initial config\n');
[lambda_no_opt,sigma_gcv_no_opt] = generalized_cross_validation(J_3axis_no_opt,dy,lambda_vector);
...
func_mdeit = @(imgh,imgi,num_noise_repetitions) noisy_data_generator_mdeit_3(imgh_nbc,imgi_nbc,snr,num_noise_repetitions);

fprintf('\t Doing Jacobian SVD\n')
...
std_sigma_mdeit_no_opt = noise_correction(imgh,imgi,J_3axis_no_opt,lambda_opt,func_mdeit,num_noise_repetitions,U_no_opt,S_no_opt,V_no_opt);
...
box on;
view(0,90)
axis([-R0 R0 -R0 R0 0 fmdl_height])
...
subplot(3,3,5)
img_temp.elem_data = sigma_gcv_opt./std_sigma_mdeit_opt;
show_fem_transparent_edges(img_temp);
eidors_colourbar(img_temp);
plot_sensors(img_out)
box on;
view(0,90)
axis([-R0 R0 -R0 R0 0 fmdl_height])
title('GCV Tykhonov Prior - opt (Noise Corrected)','Interpreter','latex')

subplot(3,3,6)
img_temp.elem_data = sigma_gcv_no_opt./std_sigma_mdeit_no_opt;
show_fem_transparent_edges(img_temp);
eidors_colourbar(img_temp);
plot_sensors(img_temp)
box on;
view(0,90)
axis([-R0 R0 -R0 R0 0 fmdl_height])
title('GCV Tykhonov Prior - no opt (Noise Corrected)','Interpreter','latex')
...
function dJ = compute_dJxyz_xyz_optimized(img)
% Computes derivative of the Jacobian w.r.t. sensor positions
% Vectorized over elements and stimulations, loop only over sensors
...
for p = 1:3
    DLx{p} = reshape((G.Gx * dlambda{p}).',[n_sensors 1 n_elem]);   % n_elem × num_sensors
...
for dim = 1:3
    for p = 1:3
        ...
        tmp = DLx{p} .* GxU + ...
            DLy{p} .* GyU + ...
            DLz{p} .* GzU;   % n_elem × num_sensors × num_stim (implicit)
...
function dphidp = compute_cost_function_gradient_a_opt_optimized_3_axis_changed(...
    img,sensor_locations,Gamma_prior_inv,Gamma_noise_inv,A)
