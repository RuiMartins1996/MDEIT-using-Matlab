...
model_parameters.material.position = ...
    [3*model_parameters.radius/4, 0, model_parameters.height/2];
...
model_parameters_recon.maxsz = model_parameters_recon.radius/8;
...
centroid_dist = sqrt(sum((fmdl_recon.elem_centroids - anomaly_center).^2,2));
...
% Mesh quadrature geometry (depends only on the mesh)
[ctx.xi,ctx.detJ,ctx.weights] = precompute_quad_geometry(fmdl);
end
...
R = compute_R(ctx,sensor_locations);
...
GzL = reshape(G.Gz*lambda,[n_elem 1 numSensors]);

GxU = reshape(ctx.GxU,[n_elem num_stim 1]);
GyU = reshape(ctx.GyU,[n_elem num_stim 1]);
GzU = reshape(ctx.GzU,[n_elem num_stim 1]);
...
dRy = compute_dR(ctx,sensor_locations,2);
dRx = compute_dR(ctx,sensor_locations,1);
...
function R = compute_R(ctx,sensor_locations)
xi = ctx.xi;  detJ = ctx.detJ;
...
function dR = compute_dR(ctx,sensor_locations,j)
xi = ctx.xi;  detJ = ctx.detJ;
numElements = size(xi,3);
numSensors  = size(sensor_locations,1);
numQuadPts  = size(xi,1);
...
%% Mesh-only quadrature geometry (quad points xi, |det J| per element)
function [xi,detJ,weights] = precompute_quad_geometry(fmdl)
...
%% Biot-Savart element integrals (used by assign_sensor_locations, arbitrary fmdl)
