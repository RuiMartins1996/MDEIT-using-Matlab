...
model_parameters.maxsz = min(model_parameters.height,model_parameters.radius)/8;
...
function x = map_q_to_x_xi(q,rmin,rmax,theta0,z0)
n_sensors = numel(q);
...
% Map x-vector to generalized coordinates q (xi,theta) and back
function x = map_q_to_x_xi_theta(q,rmin,rmax,z0)
n_sensors = numel(q)/2;
...
q_to_x_xi_theta = @(q) map_q_to_x_xi_theta(q,rmin,rmax,z0);
...
if ~exist('data/sensor_positions_opt.mat','file')
    ...
    img_out = optimize_sensor_configuration(imgsh{1},inv_Gamma_prior_3_axis,inv_Gamma_noise_3_axis,...
        jac_coord_transf_xi_theta,q_to_x_xi_theta,x_to_q_xi_theta,'a-opt','mdeit3',3,options);
    imgout{1} = img_out;
else
    imgsh{1} = imgh;
